...
import { testRedisConnection, testDatabaseConnection } from '@/app/actions/health-check';
  ...
  return (
    <>
      ...
      <section className="mb-8 bg-white rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold mb-4">Redis Health Check (Admin Only)</h2>
        ...
        {redisResult && (
          <div className={`p-4 rounded-lg ${redisResult.success ? 'bg-green-50' : 'bg-red-50'}`}>
            <h3 className={`font-semibold mb-2 ${redisResult.success ? 'text-green-800' : 'text-red-800'}`}>
              {redisResult.success ? '✓ Redis Connected' : '✗ Redis Connection Failed'}
            </h3>
            
            {redisResult.success && redisResult.data && (
              <dl className="grid grid-cols-2 gap-2 text-sm">
                <dt className="font-medium text-gray-600">Write Time:</dt>
                <dd className="text-gray-900">{redisResult.data.writeTime}ms</dd>
                ...
                <dd className="text-gray-900">{redisResult.data.deleteTime}ms</dd>
                
                <dt className="font-medium text-gray-600">Total Time:</dt>
                <dd className="text-gray-900 font-semibold">{redisResult.data.totalTime}ms</dd>
              </dl>
            )}
      ...
      <section className="mb-8 bg-white rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold mb-4">Database Health Check (Admin Only)</h2>
        ...
        {dbResult && (
          <div className={`p-4 rounded-lg ${dbResult.success ? 'bg-green-50' : 'bg-red-50'}`}>
            <h3 className={`font-semibold mb-2 ${dbResult.success ? 'text-green-800' : 'text-red-800'}`}>
              {dbResult.success ? '✓ Database Connected' : '✗ Database Connection Failed'}
            </h3>
            
            {dbResult.success && dbResult.data && (
              <dl className="grid grid-cols-2 gap-2 text-sm">
                <dt className="font-medium text-gray-600">Database:</dt>
                <dd className="text-gray-900 text-xs">{dbResult.data.databaseInfo}</dd>
                ...
                <dt className="font-medium text-gray-600">Total Time:</dt>
                <dd className="text-gray-900 font-semibold">{dbResult.data.totalTime}ms</dd>
              </dl>
            )}
