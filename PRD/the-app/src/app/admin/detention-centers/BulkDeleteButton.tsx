...
export default function BulkDeleteButton({ state, label, className = '' }: BulkDeleteButtonProps) {
    ...
    try {
      ...
      if (result.success) {
        if (result.deletedCount > 0) {
          alert(`Successfully deleted ${result.deletedCount} empty detention center(s)`);
        } else {
          alert('No empty detention centers found to delete');
        }
      } else if (result.errors) {
        alert(`Error: ${result.errors._form?.[0] || 'Failed to delete detention centers'}`);
      }
    } catch (error) {
