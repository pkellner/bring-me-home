...
interface MultiLanguageStoryEditorProps {
  personId?: string;
  stories?: Story[];
  onChange: (stories: { language: string; storyType: string; content: string }[]) => void;
}
...
export default function MultiLanguageStoryEditor({ personId, stories = [], onChange }: MultiLanguageStoryEditorProps) {
  const [selectedLanguage, setSelectedLanguage] = useState('en');
  const [selectedStoryType, setSelectedStoryType] = useState('personal');
  const [storyContents, setStoryContents] = useState<Record<string, Record<string, string>>>({});
  ...
  const handleContentChange = (content: string) => {
    ...
    const storiesArray: { language: string; storyType: string; content: string }[] = [];
  ...
  const currentContent = storyContents[selectedLanguage]?.[selectedStoryType] || '';

  const getLanguagesWithContent = (storyType: string) => {
    return languageOptions.filter(lang => 
      storyContents[lang.code]?.[storyType] && storyContents[lang.code][storyType].trim()
    );
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gray-50 p-4 rounded-lg">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Multi-Language Story Editor</h3>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {languageOptions.map(lang => {
              const hasContent = storyContents[lang.code]?.[selectedStoryType]?.trim();
              return (
                <button
                  key={lang.code}
                  type="button"
        ...
        <div className="text-sm text-gray-500 mb-4">
          Currently editing: <span className="font-medium">{storyTypes.find(t => t.type === selectedStoryType)?.label}</span> in{' '}
          <span className="font-medium">{languageOptions.find(l => l.code === selectedLanguage)?.name}</span>
        </div>
      </div>
      ...
      <div>
        <RichTextEditor
          ...
          placeholder={`Enter the ${selectedStoryType} story in ${languageOptions.find(l => l.code === selectedLanguage)?.name}...`}
          height={400}
        />
      </div>
      ...
      <div className="bg-blue-50 p-4 rounded-lg">
        <h4 className="text-sm font-medium text-blue-900 mb-2">Translation Summary</h4>
